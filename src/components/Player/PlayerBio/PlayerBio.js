...
class PlayerBio extends Component {
    componentDidMount() {
        Sources.getPlayerBio(this.props.id)
            .then(res => {
                if (res.data.error) {
                ...
                this.props.setPlayerName(
                    res.data.CommonPlayerInfo[0].display_first_last
                );
        ...
        Object.keys(bioHeadersMap).forEach(header => {
            headers.push(
                <div className="player-bio-header" key={header}>
                    <h5 className="player-bio-left-headers">
                        {bioHeadersMap[header]}:
                    </h5>
                ...
                {!this.props.playerError ? (
                    <div>
                        <h1 className="player-bio-name">
                            <Link
                                to={`/players/${this.props.id}`}
                                className="player-bio-name-link"
                            >
                                {this.props.playerName}
                            </Link>
                        </h1>
                        {Object.keys(this.props.playerInfo).length > 0 ? (
                            this.addBioHeaders()
                        ) : (
                            <Loader active style={{ minHeight: '30px' }} />
                        )}
                        <hr className="bio-divider" />
                    </div>
                ) : (
                    <h2>Error while fetching player.</h2>
                )}
            </div>
        );
    }
