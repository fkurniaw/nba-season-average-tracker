...
import {
    ...
    cellsToSkip,
    nonPerGameFields,
} from './playerGameLogConstants.js';
...
class PlayerGameLog extends React.Component {
    ...
    componentDidMount() {
        ...
        Sources.getGameLog(
            ...
            this.props.seasonType
        )
            .then(res => {
                this.props.setPlayerGameLog(
                    res.data.PlayerGameLog,
                    this.props.seasonType
                );
                this.props.setPlayerCumulativeAverageGameLog(
                    res.data.CumulativeAverageGameLog,
                    this.props.seasonType
                );
                this.props.setPlayerCumulativeTotalGameLog(
                    res.data.CumulativeTotalGameLog,
                    this.props.seasonType
                );
    ...
    renderGameLogChart(chartType, gameLogType) {
        ...
        chartType += !nonPerGameFields.includes(chartType)
            ? labelAffix[gameLogType]
            : '';
        let data = [];
        ...
        dropdownOptions =
            Object.keys(this.props.missingFields).length > 0
                ? dropdownOptions.filter(
                      option =>
                          !this.props.missingFields.missingFieldsGameLog[
                              option.key
                          ]
                  )
                : dropdownOptions;
    ...
    renderGameLogTabs() {
        let seasonType =
            this.props.seasonType === 'regularSeason'
                ? 'Regular Season'
                : 'Post Season';
        let titles = [
            `${seasonType} Game Log`,
            `Cumulative ${seasonType} Averages Game Log`,
        ...
        let panes = [
            {
                ...
                render: () => (
                    <Tab.Pane className="player-game-log-tab">
                        {this.props.playerGameLog.length > 0 &&
                            this.renderGameLogChart(
                                this.state.chartType,
                                'playerGameLog'
                            )}
                        {this.renderGameLog(
                            `${titles[0]} (${this.props.match.params.season})`
                        )}
                    </Tab.Pane>
                ),
            },
            {
                ...
                render: () => (
                    <Tab.Pane className="player-game-log-tab">
                        {this.props.playerCumulativeAverageGameLog.length > 0 &&
                            this.renderGameLogChart(
                                this.state.chartType,
                                'playerCumulativeAverageGameLog'
                            )}
                        {this.renderCumulativeAverages(
                            `${titles[1]} (${this.props.match.params.season})`
                        )}
                    </Tab.Pane>
                ),
            },
            {
                ...
                render: () => (
                    <Tab.Pane className="player-game-log-tab">
                        {this.props.playerCumulativeTotalGameLog.length > 0 &&
                            Object.keys(this.props.missingFields).length > 0 &&
                            this.renderGameLogChart(
                                this.state.chartType,
                        ...
                        {this.renderCumulativeTotals(
                            `${titles[2]} (${this.props.match.params.season})`
                        )}
                    </Tab.Pane>
                ),
            },
...
const mapStateToProps = (state, ownProps) => {
    return {
        ...
        playerCumulativeAverageGameLog:
            ...
            [],
        playerCumulativeTotalGameLog:
            state.players[ownProps.seasonType].playerCumulativeTotalGameLog,
    };
};
...
const actionCreators = {
    ...
    setPlayerCumulativeAverageGameLog:
        actions.setPlayerCumulativeAverageGameLog,
