...
const PlayerGameLogCumulativeAverage = props => {
  ...
  let rows = props.playerCumulativeAverageGameLog.map((game, i) => {
    ...
    props.statsFields.forEach((field, j) => { // add all stats other than game number
      ...
      if (j > 2 && props.playerCumulativeAverageGameLog.length > props.minGames && game.game_num >= props.minIndex &&
        game[field] !== null && maxes[j - 3].val <= game[field] && ((game[field] !== 0 && field !== 'plus_minus') || field === 'plus_minus')) {
