...
const cumulativeFiltering = function(playerGameLog) {
    const averages = [];
    const totals = [];
    const validCount = {}; // for tracking averages of normal counting stats when some are missing (prevents average from dropping in case of missed stat)
    const validPctTotals = {
        fgm: 0,
        fga: 0,
        fg3m: 0,
        fg3a: 0,
    ...
    const missingFieldsAverages = {};
    const missingFieldsGameLog = {};
    playerGameLog.forEach((game, i) => {
        ...
        statFields.forEach((field, j) => {
            if (i === 0 || j < nonAvgFieldsIndex) {
                ...
                averages[i][field] =
                    i > 0 ? averages[i - 1][field] || null : null;
            } else {
                validCount[field]++;
                if (!cellsToSkip.includes(field)) {
                    ...
                    averages[i][field] =
                        averages[i - 1][field] *
                            ((validCount[field] - 1) / validCount[field]) +
                        game[field] * (1 / validCount[field]);
                } else {
                    if (field === 'fg_pct')
...
function calculatePercentages(
    ...
    if (game[makes] !== null && game[attempts] !== null) {
        ...
        totals[gameNum][pctType] =
            validPctTotals[attempts] !== 0
                ? validPctTotals[makes] / validPctTotals[attempts]
                : 0;
